...
Region MetaGraph::getBoundingBox() const
{
   Region bounds = SuperSpacePixel::getRegion();
...
bool MetaGraph::makePoints( const Point2f& p, int semifilled, Communicator *communicator )
{
//   m_state &= ~POINTS;

   try {
      getDisplayedPointMap().makePoints( p, semifilled, communicator );
   }
...
bool MetaGraph::analyseGraph( Communicator *communicator, Options options )   // <- options copied to keep thread safe
{
   bool retvar = false;
   ...
   try {
      ...
      if (options.point_depth_selection == 1) {
         ...
         PointMaps::getDisplayedPointMap().analyseIsovist( communicator, *this );
      }
      else if (options.output_type == Options::OUTPUT_VISUAL) {
         PointMaps::getDisplayedPointMap().analyseVisual( communicator, options );
         // REPLACES:
...
// returns 0: fail, 1: made isovist, 2: made isovist and added new shapemap layer
int MetaGraph::makeIsovistPath(Communicator *communicator, double fov)
{
   int retvar = 0;
...
bool MetaGraph::analyseAxial( Communicator *communicator, Options options ) // options copied to keep thread safe
{
   m_state &= ~SHAPEGRAPHS;      // Clear axial map data flag (stops accidental redraw during reload) 
   ...
   try {
      ...
      retvar = m_shape_graphs.getDisplayedMap().integrate( communicator, radius, options.choice, options.local, options.fulloutput, options.weighted_measure_col );
   } 
   catch (Communicator::CancelledException) {
      retvar = false;
   }
...
int MetaGraph::loadDxf( istream& stream, Communicator *communicator)
{
   ...
   SuperSpacePixel::tail().m_region = Region(dp.getExtMin(), dp.getExtMax());
   ...
   for (int layerref = 0; layerref < dp.numLayers(); layerref++) {
      ...
      SuperSpacePixel::tail().at(i).init(dxf_layer.numTotalPoints() + dxf_layer.numTotalLines() + dxf_layer.numArcs() * DXFCIRCLERES + dxf_layer.numCircles() * DXFCIRCLERES, Region(Point2f(dxf_layer.getExtMin()),Point2f(dxf_layer.getExtMax())) );
...
int MetaGraph::loadCat( istream& stream, Communicator *communicator )
{
   ...
   SuperSpacePixel::tail().m_region = Region(min_point, max_point);
   SuperSpacePixel::tail().push_back(ShapeMap());
   SuperSpacePixel::tail().tail().init( numlines, Region(min_point, max_point) );
...
int MetaGraph::loadRT1(const pqvector<wstring>& fileset, Communicator *communicator)
{
   ...
   SuperSpacePixel::tail().m_region = Region(map.getBottomLeft(), map.getTopRight());
