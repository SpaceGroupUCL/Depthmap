...
void AxialPolygons::init(prefvec<Line>& lines, const Region& region)
{
...
int ShapeGraphs::convertDataToAxial(Communicator *comm, const pstring& name, ShapeMap& shapemap, bool copydata)
{
   ...
   Region region = shapemap.getRegion();
...
bool ShapeGraph::integrate(Communicator *comm, const pvecint& radius_list, bool choice, bool local, bool fulloutput, int weighting_col)
{
...
void TidyLines::tidy(prefvec<Line>& lines, const Region& region)
{
   size_t i = 0;
   m_region = region;
   double maxdim = __max(m_region.width(),m_region.height());
...
void TidyLines::quicktidy(pqmap<int,Line>& lines, const Region& region)
{
   m_region = region;
