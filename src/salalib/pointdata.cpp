...
bool PointMap::blockLines()
{
   ...
   for (int i = 0; i < m_cols; i++) {
      for (int j = 0; j < m_rows; j++) {
         ...
         Region viewport = regionate( curs, 1e-10 );
...
void PointMap::addLineDynamic(LineKey ref,const Line& line)
{
   ...
   for (n = 0; n < pixels.size(); n++) 
   {
      ...
      Region viewport = regionate( pixels[n], 1e-10 );
...
bool PointMap::makePoints(const Point2f& seed, int semifilled, Communicator *comm)
{
   if (!m_spacepix) {
      return false;
   }
   ...
   int filltype = semifilled ? Point::FILLED | Point::CONTEXTFILLED : Point::FILLED;
...
void PointMap::makeViewportPoints( const Region& viewport ) const
{
...
bool PointMap::setCurSel(Region &r, bool add )
{
   if (m_selection == NO_SELECTION) {
      add = false;
   }
   ...
   r = Region( depixelate(s_bl), depixelate(s_tr) );
...
bool PointMap::sparkPixel2(PixelRef curs, int make, double maxdist)
{
   ...
   for (int q = 0; q < 8; q++) {
      ...
      Region viewport0 = regionate(curs, 1e-10); 
      switch (q) {
      case 0:
...
bool PointMap::analyseIsovist(Communicator *comm, MetaGraph& mgraph)
{
   ...
   for (int i = 0; i < m_cols; i++) {
      for (int j = 0; j < m_rows; j++) {
         ...
         if (getPoint( curs ).filled()) {
            ...
            int row = m_attributes.getRowid(curs);
            isovist.setData(m_attributes,row);
...
bool PointMap::analyseVisual(Communicator *comm, Options& options)
{
   __time64_t atime = 0;
   if (comm) {
      qtimer( atime, 0 );
