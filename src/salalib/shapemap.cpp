...
void ShapeMap::init(int size, const Region &r)
{
   if (m_pixel_shapes) {
      for (int i = 0; i < m_cols; i++) {
         delete [] m_pixel_shapes[i];
...
int ShapeMap::makePointShape(const Point2f& point, bool tempshape)
{
   ...
   if (!m_region.contains_touch(point)) {
      bounds_good = false;
      init(m_shapes.size(),Region(point,point));
   }
...
int ShapeMap::makePolyShape(const pvecpoint& points, bool open, bool tempshape)
{
   ...
   Region region(points[0],points[0]);
   size_t i;
...
int ShapeMap::makeShapeFromPointSet(const PointMap& pointmap)
{
   ...
   if (!bounds_good) {
      Region r(pointmap.getRegion().bottom_left - offset,pointmap.getRegion().top_right + offset);
...
int ShapeMap::shapeEnd(bool open)
{
   ...
   if (m_region.isNull()) {
      m_region = Region(m_temppoints[0],m_temppoints[0]);
   }
   for (int i = 0; i < len; i++) {
...
bool ShapeMap::setCurSel( Region& r, bool add )
{
   if (add == false) {
      clearSel();
   }
   ...
   while (!stream.eof()) {
      ...
      if (!inputline.empty()) {
         ...
         if (pts) {
            points.push_back(p1);
            if (points.size() == 1) {
               region = Region(points.tail(),points.tail());
            }
            else {
               region = runion(region,Region(points.tail(),points.tail()));
            }
         }
         else {
...
void ShapeMap::makeViewportShapes( const Region& viewport ) const
{
   if (m_invalidate) {
      return;
   }
...
pqvector<SalaEdgeU> SalaShape::getClippingSet(Region& clipframe) const
{
...
void ShapeMap::ozlemSpecial(ShapeMap& output)
{
   ...
   output.init(m_shapes.size(),Region(m_region.bottom_left+Point2f(-50.0,-50.0),m_region.top_right+Point2f(50.0,50.0)));
      ...
      for (size_t j = 0; j < sr.size(); j++) {
         if (sr[j].m_shape_ref != m_shapes.key(i)) {
            if (m_shapes[i].getPoint() == m_shapes.search(sr[j].m_shape_ref).getPoint()) {
               duplicate = true;
               if (m_shapes.key(i) > sr[j].m_shape_ref) {
                  tag_delete = true;
               }
            }
         }
