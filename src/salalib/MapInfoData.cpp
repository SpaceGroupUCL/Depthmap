...
int MapInfoData::import(istream& miffile, istream& midfile, ShapeMap& map)
{
   ...
   Region region(pointsets[0][0],pointsets[0][0]);
...
bool MapInfoData::exportPolygons(ostream& miffile, ostream& midfile, const prefvec<pvecpoint>& polygons, const Region& region)
{
   // if bounds has not been filled in, fill it in
   if (m_bounds.empty()) {
      char bounds[256];
