...
public:
   ...
   { return m_region; }
   const Region& getBoundingBox() const
   { return m_region; }
   //
   ...
   //
   pqvector<SalaEdgeU> getClippingSet(Region& clipframe) const;
   //
...
public:
   ...
   //
   //
   void init(int size, const Region& r);
...
public:
   ...
   { return m_selection; }
   bool setCurSel( Region& r, bool add = false );
...
public:
   // Screen
   void makeViewportShapes( const Region& viewport ) const;
...
template <class T>
class ShapeMaps : protected prefvec<T>
{
protected:
...
public:
   ...
   { return at(m_displayed_map).getObjectCount(); }
   const size_t getShapeCount() const
   { return at(m_displayed_map).m_shapes.size(); }
   //
   bool read( ifstream& stream, int version );
   bool write( ofstream& stream, int version, bool displayedmaponly = false );
   //
   const Region& getBoundingBox() const
   { return at(m_displayed_map).getRegion(); }
   //
   // making links and unlinks
   bool linkShapes(const Point2f& p)
   { return at(m_displayed_map).linkShapes(p); }
   bool unlinkShapes(const Point2f& p)
   { return at(m_displayed_map).unlinkShapes(p); }
   // for displaying links and unlinks
   bool findNextLinkLine() const
   { return at(m_displayed_map).findNextLinkLine(); }
   Line getNextLinkLine() const
   { return at(m_displayed_map).getNextLinkLine(); }
   // unlinks are actually only used for special case of axial lines:
   bool findNextUnlinkPoint() const
   { return at(m_displayed_map).findNextUnlinkPoint(); }
   Point2f getNextUnlinkPoint() const
   { return at(m_displayed_map).getNextUnlinkPoint(); }
};
...
{ 
   ...
   remove_at(map); 
   if (m_displayed_map > map || m_displayed_map >= size()) 
      m_displayed_map--; 
}
template <class T>
...
void ShapeMaps<T>::setDisplayedMapRef(size_t map)
{
   if (m_displayed_map != paftl::npos && m_displayed_map != map)
      at(m_displayed_map).clearSel();
   m_displayed_map = map;
}
template <class T>
bool ShapeMaps<T>::read( ifstream& stream, int version )
{
   clear(); // empty existing data
