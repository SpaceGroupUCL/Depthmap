...
public:
   ...
   //
   void clear();
   void init(prefvec<Line>& lines, const Region& region);
...
public:
   ...
   bool integrate(Communicator *comm = NULL, const pvecint& radius = pvecint(), bool choice = false, bool local = false, bool fulloutput = false, int weighting_col = -1);
...
public:
   TidyLines() {;}
   virtual ~TidyLines() {;}
   void tidy(prefvec<Line>& lines, const Region& region);  
   void quicktidy(pqmap<int,Line>& lines, const Region& region);
};
