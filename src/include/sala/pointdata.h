...
public:
   ...
   Region regionate( const PixelRef& p, double border ) const;     // Inlined below
   ...
   //
   bool makePoints(const Point2f& seed, int semifilled, Communicator *comm = NULL); // Point2f non-reference deliberate
   ...
   bool analyseIsovist(Communicator *comm, MetaGraph& mgraph);
   bool analyseVisual(Communicator *comm, Options& options);
...
public:
   ...
   bool setCurSel( Region& r, bool add = false ); // set current selection
...
protected:
   ...
   mutable Region m_sel_bounds;
public:
   void setScreenPixel( double m_unit );
   void makeViewportPoints( const Region& viewport ) const;
   ...
   const Region& getSelBounds() const
      { return m_sel_bounds; }
   //
...
inline Region PointMap::regionate( const PixelRef& p, double border ) const
{
   return Region(
