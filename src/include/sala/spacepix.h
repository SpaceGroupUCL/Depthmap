...
//   PixelRefList pixelate( const Line& l ) const;
   ...
   //
   virtual void makeViewportLines( const Region& viewport ) const;
...
public:
   Region m_region;  // easier public for now
   //
   ...
   void makeViewportShapes( const Region& viewport = Region() ) const;
   bool findNextShape(bool& nextlayer) const;
   const SalaShape& getNextShape() const
      { return at(m_current_layer).getNextShape(); }
   const PafColor getLineColor() const
      { return at(m_current_layer).getLineColor(); }
   const int getLineStyle() const
      { return at(m_current_layer).getLineStyle(); }
   const bool getLineSelected() const
      { return  at(m_current_layer).getLineSelected(); }
   //
   void cutLine(Line& l);//, short dir);
   //
   // Is any one sublayer shown?
   bool isShown() const
      { for (size_t i = 0; i < size(); i++) if (at(i).isShown()) return true; return false; }
   //
public:
...
template <class T> 
void SpacePixelGroup<T>::makeViewportShapes( const Region& viewport ) const
{
   m_current_layer = -1;
   for (size_t i = size() - 1; i != paftl::npos; i--) {
      if (at(i).isShown()) {
         m_current_layer = (int) i;
         at(i).makeViewportShapes( (viewport.isNull() ? m_region : viewport) );
      }
   }
}
template <class T>
bool SpacePixelGroup<T>::findNextShape(bool& nextlayer) const
{
   if (m_current_layer == -1) 
      return false;
   while (!at(m_current_layer).findNextShape(nextlayer)) {
      while (++m_current_layer < (int)size() && !at(m_current_layer).isShown());
      if (m_current_layer == size()) {
         m_current_layer = -1;
         return false;
      }
   }
