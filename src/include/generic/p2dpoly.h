...
//      { return x == P2DNULL || y == P2DNULL; }
      { return x == 0.0 && y == 0.0; }
   void normalScale( const Region& );  // inline function: below region
   void denormalScale( const Region& );
...
public:
   Point2f bottom_left;
   Point2f top_right;
   Region(const Point2f& bl = Point2f(), const Point2f& tr = Point2f())
   ...
   double area() const
      { return height() * width(); }
   void normalScale( const Region& r )
      { top_right.normalScale(r); bottom_left.normalScale(r); }
   void denormalScale( const Region& r )
   ...
   //
   friend bool intersect_region(const Region& a, const Region& b, double tolerance = 0.0);
   friend bool overlap_x(const Region& a, const Region& b, double tolerance = 0.0);
   friend bool overlap_y(const Region& a, const Region& b, double tolerance = 0.0);
   //
   // set functions
   friend Region runion(const Region& a, const Region& b);
   friend Region rintersect( const Region& a, const Region& b); // undefined?
   //
...
inline void Point2f::normalScale( const Region& r )
{
   x = (x - r.bottom_left.x) / r.width();
   y = (y - r.bottom_left.y) / r.height();
}
inline void Point2f::denormalScale( const Region& r )
{
...
public:
   ...
   Line(const Region& r) : Region(r)
      { bits.parity = 1; bits.direction = 1; }
   Line(const Line& l) : Region(l)
      { bits = l.bits; }
   Line& operator = (const Line& l)
      { this->Region::operator = (l); bits = l.bits; return *this; }
   //
   ...
   //
   bool crop(const Region& r);
   void ray(short dir, const Region& r);
   //
...
public:
   ...
   Region get_bounding_box()
      { return *(Region *)(m_p_root->m_p_region); }
   //
