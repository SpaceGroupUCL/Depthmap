...
Point2f Region::getEdgeUPoint(const EdgeU& eu)
{
   switch (eu.edge) {
      case 0:
...
EdgeU Region::getCutEdgeU(const Point2f& inside, const Point2f& outside)
{
   EdgeU eu;
...
Region runion(const Region& a, const Region& b)
{
   Region n;
...
bool intersect_region(const Region& a, const Region& b, double tolerance)
{
...
bool overlap_x(const Region& a, const Region& b, double tolerance)
{
...
bool overlap_y(const Region& a, const Region& b, double tolerance)
{
...
bool intersect(const RegionTree& a, const RegionTree& b)
{
   if (a.is_leaf() && b.is_leaf()) {
      if (intersect_region( Region(a), Region(b))) {
         return intersect_line( (const Line&) Region(a), (const Line&) Region(b) );
      }
      else {
         return false;
      }
...
void Line::ray(short dir, const Region& r)
{
...
void Poly::add_line_segment(const Line& l)
{
   ...
   else
   {
      ...
      if (cut_level < 0)
      {
         ...
         Region new_region =
            runion( Region(*m_p_root), Region(*leaf) );
         RegionTree *new_root =
     ...
     else
     {
         ...
         Region new_region =
            runion( Region(insertion_point), Region(*leaf) );
         ...
         while (cut_level >= 0)
         {
            ...
            here->m_p_region = new Line(
               runion(Region(here->left()), Region(here->right())) );
            cut_level--;
         }
      }
   }
